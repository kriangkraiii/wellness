...
const READINESS_ITEMS = [
  "แจ้งสถานะสุขภาพปัจจุบันถูกต้อง",
  "รับประทานอาหารก่อนนวด 1–2 ชั่วโมง",
  "ไม่ดื่มแอลกอฮอล์/กาแฟก่อนนวด",
  "ไม่มีอาการไข้หรือการอักเสบรุนแรง",
  "ไม่ได้รับการผ่าตัดภายใน 3 เดือน",
  "ไม่ปวดตัวรุนแรงก่อนนวด",
];

const CAUTION_ITEMS = [
  "ห้ามนวดบริเวณที่มีแผลเปิด",
  "ระวังบริเวณที่มีรอยช้ำ",
  "ระวังความดันโลหิตสูง",
  "โรคหัวใจ",
  "ภาวะกระดูกพรุน",
  "เส้นเลือดดำขอด",
  "อื่นๆ",
...
const MASSAGE_TECHNIQUES = [
  { id: "thai", label: "นวดไทย", icon: "" },
  { id: "oil", label: "นวดน้ำมัน", icon: "" },
  { id: "hot-compress", label: "ประคบ", icon: "" },
  { id: "acupressure", label: "กดจุด", icon: "" },
...
const SELF_CARE_TIPS = [
  {
    icon: "",
    title: "ดื่มน้ำมากๆ",
    detail: "ดื่มน้ำอย่างน้อย 1.5–2 ลิตรต่อวัน",
  },
  {
    icon: "",
    title: "หลีกเลี่ยงการออกกำลังกายหนัก",
    detail: "อย่างน้อย 24–48 ชั่วโมง",
  },
  { icon: "", title: "ยืดกล้ามเนื้อเบาๆ", detail: "วันละ 10–15 นาที" },
  { icon: "", title: "ประคบอุ่น", detail: "ประคบบริเวณที่นวด 1–2 ครั้ง/วัน" },
  {
    icon: "",
    title: "บอกนักนวดหากไม่สบายตัว",
    detail: "บอกทันที หากมีอาการผิดปกติ 6–8 ชั่วโมง",
  },
];
      ...
      {STEP_TITLES.map((_, i) => {
        ...
        return (
          <div key={idx} className="flex items-center">
            <div
              className={`flex h-7 w-7 items-center justify-center rounded-full text-[10px] font-bold transition-all ${active ? "bg-[var(--gold)] text-[#1a1a0e] shadow-[0_0_12px_var(--gold-glow)]" : done ? "bg-[var(--gold)] text-[#1a1a0e]" : "bg-[var(--bg-surface)] text-[var(--ink-muted)]"}`}
            >
              {done ? (
            ...
            {i < STEP_TITLES.length - 1 && (
              <div
                className={`h-0.5 w-6 ${done ? "bg-[var(--gold)]" : "bg-[var(--line)]"}`}
              />
            )}
          </div>
        );
    ...
    <header className="sticky top-0 z-10 border-b border-[var(--line)] bg-[var(--bg-deep)]/95 px-4 pb-3 pt-3 backdrop-blur">
      <div className="flex items-center justify-between">
        <button
          type="button"
          onClick={onBack}
          className="flex h-9 w-9 items-center justify-center rounded-full border border-[var(--line)] bg-[var(--bg-surface)] text-[var(--ink-soft)] transition hover:text-[var(--gold)]"
          aria-label="ย้อนกลับ"
        >
          <svg
            viewBox="0 0 24 24"
        ...
        >
          {submitting && (
            <svg
              className="h-3 w-3 animate-spin text-[var(--gold)]"
              fill="none"
              viewBox="0 0 24 24"
            >
              <circle
...
function ReadinessRow({
  ...
  return (
    <div className="flex items-center justify-between border-b border-[var(--line)] py-2.5 last:border-0">
      <span className="flex-1 pr-2 text-xs text-[var(--ink-soft)]">
        {label}
      </span>
...
function IntensitySlider({
  ...
  const barColor =
    value >= 8 ? "var(--danger)" : value >= 5 ? "var(--warn)" : "var(--accent)";
  return (
...
export function TherapistForm() {
  ...
  const [readiness, setReadiness] = useState<Record<string, ReadinessStatus>>(
    Object.fromEntries(READINESS_ITEMS.map((item) => [item, "na"])),
  );
  const [cautions, setCautions] = useState<string[]>([]);
  const [note1, setNote1] = useState("");
  const [prePoints, setPrePoints] = useState<SelectedBodyPoint[]>([]);
  const [preIntensity, setPreIntensity] = useState<Record<string, number>>(
    Object.fromEntries(PAIN_AREAS.map((a) => [a, 0])),
  );
  const [postPoints, setPostPoints] = useState<SelectedBodyPoint[]>([]);
  const [postIntensity, setPostIntensity] = useState<Record<string, number>>(
    Object.fromEntries(PAIN_AREAS.map((a) => [a, 0])),
  );
      ...
      try {
        ...
        if (data.ok) {
          ...
          if (data.records.length > 0) {
            const queryMatched = queryRecordId
              ? data.records.find((r: CustomerRecord) => r.id === queryRecordId)
              : null;
            const pending =
              queryMatched ||
              data.records.find((r: CustomerRecord) => !r.therapistRecord) ||
              data.records[0];
  ...
  function handleCustomerSelect(record: CustomerRecord) {
    ...
    if (record.bodyPoints) {
      try {
        const points =
          typeof record.bodyPoints === "string"
            ? JSON.parse(record.bodyPoints)
            : record.bodyPoints;
        setPrePoints(points || []);
      } catch (e) {
  ...
  function toggleBodyPoint(
    point: BodyPoint,
    list: SelectedBodyPoint[],
    setter: React.Dispatch<React.SetStateAction<SelectedBodyPoint[]>>,
  ) {
    setter((prev) => {
  ...
  function toggleCaution(item: string) {
    setCautions((prev) =>
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center px-5 text-center">
        <div className="rounded-full bg-[var(--gold)] p-5 shadow-[0_0_40px_var(--gold-glow)]">
          <svg
            viewBox="0 0 24 24"
            className="h-10 w-10 fill-none stroke-[#1a1a0e]"
            strokeWidth="2.5"
            strokeLinecap="round"
          >
  ...
  function renderStep1() {
    return (
      <div className="space-y-3">
        <div className="glass-card p-4">
          <label className="mb-2 block text-xs font-bold text-[var(--gold)] uppercase tracking-wider">
            เลือกประวัติลูกค้า
          </label>
          {loadingRecords ? (
            ...
            <p className="py-2 text-xs text-[var(--danger)]">
              ไม่มีประวัติลูกค้าในระบบ กรุณาแจ้งลูกค้ากรอกแบบฟอร์มก่อน
            </p>
          ) : (
            <select
              value={selectedRecordId}
              onChange={(e) => {
                const rec = customerRecords.find(
            ...
            >
              {customerRecords.map((rec) => (
                <option key={rec.id} value={rec.id}>
                  {rec.name} ({rec.age} ปี, {rec.gender}) -{" "}
                  {new Date(rec.createdAt).toLocaleDateString("th-TH")}{" "}
                  {rec.therapistRecord ? "✓ ประเมินแล้ว" : " รอประเมิน"}
                </option>
              ))}
            </select>
          )}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-[var(--ink-soft)]">
                        อายุ {rec.age} ปี | เพศ {rec.gender} | สัญชาติ{" "}
                        {rec.nationality}
                      </p>
                      <p className="text-xs text-[var(--ink-muted)]">
                        วันที่ลงทะเบียน:{" "}
                        {new Date(rec.createdAt).toLocaleString("th-TH")}
                      </p>
                    </div>
                  </div>
        ...
        <SectionCard title="1. ความพร้อมในการรับบริการ">
          {READINESS_ITEMS.map((item) => (
            <ReadinessRow
              key={item}
              label={item}
              value={readiness[item] ?? "na"}
              onChange={(v) => setReadiness((prev) => ({ ...prev, [item]: v }))}
            />
          ))}
        </SectionCard>
        <SectionCard title="2. ข้อห้าม / ข้อควรระวัง">
          <div className="grid grid-cols-2 gap-2">
            {CAUTION_ITEMS.map((item) => (
              ...
              >
                <span
                  className={`h-4 w-4 flex-shrink-0 rounded-full border-2 ${cautions.includes(item) ? "border-[var(--danger)] bg-[var(--danger)]" : "border-[var(--line-hover)]"}`}
                />
                {item}
              </button>
            ))}
        ...
        <SectionCard title="หมายเหตุเพิ่มเติม (ถ้ามี)">
          <textarea
            ...
            placeholder="บันทึกข้อสังเกตพิเศษ..."
  ...
  function renderStep2() {
    return (
      <div className="space-y-3">
        <div className="glass-card p-4">
          <h3 className="mb-2 text-center text-sm font-semibold text-[var(--accent)]">
            1. ประเมินอาการ (สถานภาพก่อนนวด)
          </h3>
          <div className="flex gap-2">
            <div className="flex-1 text-center">
              <p className="mb-1 text-[11px] text-[var(--ink-muted)]">
                ด้านหน้า
              </p>
              <BodyDiagram
                selected={prePoints}
                onToggle={(p) => toggleBodyPoint(p, prePoints, setPrePoints)}
                points={BODY_POINTS}
                useFront={true}
              />
            </div>
          ...
          <p className="mt-2 text-center text-[11px] text-[var(--ink-muted)]">
            เลือกแล้ว {prePoints.length} จุด — แตะซ้ำเพื่อเปลี่ยนระดับ
          </p>
        </div>
        <SectionCard title="2. ระดับความรุนแรงของอาการ (0-10)">
          {PAIN_AREAS.map((area) => (
            <IntensitySlider
              key={area}
              label={area}
              value={preIntensity[area] ?? 0}
              onChange={(v) =>
                setPreIntensity((prev) => ({ ...prev, [area]: v }))
              }
            />
          ))}
        </SectionCard>
      ...
      <div className="space-y-3">
        <div className="glass-card p-4">
          ...
          <div className="flex gap-2">
            <div className="flex-1 text-center">
              ...
              <BodyDiagram
                selected={postPoints}
                onToggle={(p) => toggleBodyPoint(p, postPoints, setPostPoints)}
                points={BODY_POINTS}
                useFront={true}
              />
            </div>
        ...
        <SectionCard title="2. ระดับความรุนแรงหลังนวด (0-10)">
          {PAIN_AREAS.map((area) => (
            <IntensitySlider
              key={area}
              label={area}
              value={postIntensity[area] ?? 0}
              onChange={(v) =>
                setPostIntensity((prev) => ({ ...prev, [area]: v }))
              }
            />
          ))}
        </SectionCard>
            ...
            {MASSAGE_TECHNIQUES.map((tech) => {
              ...
              return (
                ...
                >
                  ...
                  <span
                    className={`text-[11px] font-medium ${active ? "text-[var(--gold)]" : "text-[var(--ink-soft)]"}`}
                  >
                    {tech.label}
                  </span>
                </button>
        ...
        <div className="glass-card p-4">
          <h3 className="mb-2 text-sm font-semibold text-[var(--gold)]">
            4. จุดที่เน้น (Trigger Point)
          </h3>
          <p className="mb-3 text-xs text-[var(--ink-muted)]">
            แตะตำแหน่งที่นวดเน้นบนร่างกาย
          </p>
          <BodyDiagram
            selected={triggerPoints}
            onToggle={(p) =>
              toggleBodyPoint(p, triggerPoints, setTriggerPoints)
            }
            points={BODY_POINTS}
            useFront={true}
          />
          <div className="mt-2 flex gap-2">
            <button
              type="button"
              className="flex-1 rounded-xl border border-[var(--gold)]/40 bg-[var(--gold-glow)] py-2 text-sm font-medium text-[var(--gold)]"
            >
              จุดที่เน้น ({triggerPoints.length})
            </button>
            <button
              type="button"
              onClick={() => setTriggerPoints([])}
              className="flex-1 rounded-xl border border-[var(--line)] bg-[var(--bg-surface)] py-2 text-sm text-[var(--ink-soft)]"
            >
              ล้างข้อมูล
            </button>
          </div>
  ...
  function renderStep4() {
    const reduced =
      PAIN_AREAS.reduce((sum, a) => sum + (preIntensity[a] ?? 0), 0) >
      PAIN_AREAS.reduce((sum, a) => sum + (postIntensity[a] ?? 0), 0);
    return (
      <div className="space-y-3">
        <div className="glass-card p-4">
          <div className="flex items-center gap-3">
            <div
              className={`flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full ${reduced ? "bg-[var(--accent)]" : "bg-[var(--warn)]"}`}
            >
              {reduced ? (
                <svg
                  viewBox="0 0 24 24"
            ...
            <div>
              ...
              <p className="text-xs text-[var(--ink-soft)]">
                {reduced
                  ? "อาการดีขึ้นหลังนวด — ระดับปวดลดลงโดยรวม 60%"
                  : "ระดับอาการยังคงต้องติดตาม"}
              </p>
            </div>
          </div>
        </div>
        <SectionCard title="คำแนะนำสำหรับลูกค้า">
          <div className="space-y-2.5">
            {SELF_CARE_TIPS.map((tip) => (
              <div
                key={tip.title}
                className="flex items-start gap-3 rounded-xl border border-[var(--line)] bg-[var(--bg-surface)] p-3"
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-[var(--ink-soft)]">{tip.detail}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <SectionCard title="แผนการนวดครั้งต่อไป">
          <div className="flex items-center gap-3">
            ...
            <button
              type="button"
              className="rounded-xl border border-[var(--gold)]/40 bg-[var(--gold-glow)] px-3 py-2 text-sm font-medium text-[var(--gold)]"
            >
               นัด
            </button>
          </div>
        ...
        <SectionCard title="หมายเหตุเพิ่มเติมจาก Therapist">
          <textarea
            ...
            placeholder="บันทึกข้อสังเกตหรือสิ่งที่ควรแจ้งลูกค้า..."
            className="input-dark w-full"
            rows={4}
          />
          <p className="mt-1 text-right text-[10px] text-[var(--ink-muted)]">
            {therapistNote.length}/300
          </p>
        </SectionCard>
      </div>
    );
      ...
      <div className="fixed inset-x-0 bottom-0 bg-gradient-to-t from-[var(--bg-deep)] via-[var(--bg-deep)] to-transparent px-4 pb-5 pt-8">
        ...
        >
          {submitting ? (
            <>
              <svg
                className="h-5 w-5 animate-spin text-[#1a1a0e]"
                fill="none"
                viewBox="0 0 24 24"
              >
                <circle
