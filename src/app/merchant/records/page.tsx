...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function MerchantRecordsPage() {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <h2 className="mt-1 font-heading text-3xl font-bold text-slate-800">
      ...
      <div className="glass-card p-5 flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="relative w-full md:max-w-md">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 text-sm rounded-xl border border-slate-200 bg-slate-50/50 text-slate-700 placeholder-slate-400 focus:outline-none focus:border-accent focus:bg-white transition"
          />
        </div>
      ...
      {loading ? (
        ...
        /* Records Table */
        <div className="glass-card overflow-hidden shadow-sm border border-slate-100/80">
                    ...
                    <tr key={rec.id} className="hover:bg-slate-50/70 transition">
                      ...
                      <td className="px-6 py-4">
                        {rec.aiRecommendation ? (
                          <span className="inline-flex items-center gap-1 text-xs font-semibold text-accent bg-accent-subtle rounded-full px-2.5 py-1">
                            ✦ มีบทวิเคราะห์
                          </span>
                        ) : (
          ...
          <div className="w-full max-w-3xl bg-white h-full shadow-2xl flex flex-col overflow-hidden relative animate-in slide-in-from-right duration-300">
            ...
            <header className="border-b border-slate-100 bg-slate-50 px-6 py-4 flex items-center justify-between">
              ...
              <button
                ...
                className="flex h-9 w-9 items-center justify-center rounded-full bg-white border border-slate-200 text-slate-500 hover:bg-slate-100 transition shadow-sm"
              >
                ✕
              </button>
            </header>
